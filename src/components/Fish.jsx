...
const Fish = React.memo(({ project, index, isMobile, onSelect, boundsWidth, boundsHeight }) => {
  ...
  useEffect(() => {
    if (project.fishImage) {
      try {
        const imageUrl = new URL(`../data/images/fish/${project.fishImage}`, import.meta.url).href;
        setFishImageSrc(imageUrl);
        setUseImage(true);
      } catch (error) {
        console.warn(`Image ${project.fishImage} non trouvée, utilisation du SVG`);
        setUseImage(false);
      }
    } else {
      setUseImage(false);
  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      ...
      if (newX <= 0 || newX >= 100 - fishWidthPercent) {
        velocityX.current *= -1;
        const newScale = velocityX.current > 0 ? -1 : 1;
        setScaleX(newScale);
        if (iconRef.current) {
          iconRef.current.style.transform = `scaleX(${newScale})`;
        }
